...
#PJM -N ftllm_feb24calibrated-Ministral-3-8B-Inst_folds1to4_cn
#PJM -j
#PJM -o results/finetuned_llm/logs/pj_ftllm_feb24calibrated-Ministral-3-8B-Inst_folds1to4_cn.out
cd "$HOME/bea2026st"
...
FOLDS=1-4 conda run --no-capture-output -n llm_plus \
  ...
  -v \
  --config-name feb24calibrated-Ministral-3-8B-Inst-cn \
